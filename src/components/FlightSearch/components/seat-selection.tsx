...
const SeatSelectionCard: React.FC<PassengerDetailProps> = ({
  setActiveSeat,
  activeSeat,
  data,
}) => {
  return (
    ...
    >
      <div className="flex justify-between items-center">
        ...
        <RadioGroupItem
          className={`${
            activeSeat === data.title ? "border-primary" : "border-[#A3A7AB]"
          }`}
          value={data.title}
          id={data.title}
        />
      ...
      <p className="text-[#1B1E21] text-xs ">
        We will automatically select your seats, you can review before
        proceeding
      </p>
    </div>
  );
